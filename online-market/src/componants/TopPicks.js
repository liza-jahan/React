  ...
  return (
    <div>
      ...
      <div className="hidden lg:flex max-w[1520px] m-auto py-2 px-2">
        <Splide
          options={{ perPage: 4, gap: "0.5rem", grap: "free", arrows: false }}
        >
          {topPicks.map((item) => {
            return (
              <SplideSlide key={item.id}>
                <div className="rouded-3xl relative">
                  ...
                  <img
                    className="h-[200px] w-full object-cover rounded-3xl cursor-pointer hover:scale-105 ease-out default:300"
                    src={item.img}
                    alt={item.title}
                  />
                </div>
